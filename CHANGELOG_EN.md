...
- fix: |GitHub Actions| Require `TEMP_MAIL_JWT_SECRET` and `TEMP_MAIL_ADMIN_PASSWORDS_JSON` when rendering `worker/wrangler.toml` automatically, preventing deployments from wiping Worker vars and leaving the instance with `JWT_SECRET is not set` or missing admin passwords; update the auto-deploy docs accordingly
- fix: |GitHub Actions| Support restoring the Cloudflare `send_email` binding (`SEND_MAIL`) when rendering Worker config automatically, preventing Actions deployments from dropping send-mail capability; controlled by `TEMP_MAIL_ENABLE_SEND_MAIL_BINDING` and `TEMP_MAIL_SEND_MAIL_DOMAINS_JSON`
- fix: |GitHub Actions| Support `TEMP_MAIL_SEND_MAIL_REMOTE=true` for the `SEND_MAIL` binding, rendering the `remote = true` option required by Cloudflare Email Service
- fix: |GitHub Actions| Write Resend/SMTP values from GitHub Secrets back into Worker secrets after backend deployment, including domain-specific `RESEND_TOKEN_<DOMAIN>` tokens, so automatic deployments do not drop Resend/SMTP send-mail capability
- fix: |GitHub Actions| Change `Upstream Sync` to merge upstream `main` while preserving this fork's auto-deployment files, avoiding forced overwrites that would break future automatic updates
