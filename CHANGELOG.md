...
- perf: |用户注册| 将注册验证码邮件升级为可配置品牌的简洁模板，增加醒目的验证码卡片、5 分钟有效期和安全提醒，并为 HTML 邮件补充纯文本替代正文；公开版默认使用可靠的文本标识，可选安全的 HTTPS Logo
- feat: |认证| 为地址 JWT 与用户 JWT 增加 `typ`、`sub`、`exp` 和数据库 `token_version` 校验；地址改密或管理员重置用户密码后旧令牌立即失效，同时兼容尚未轮换的 version 0 旧令牌
- feat: |邮件 API / IMAP| 新增有界的 `/api/mail_ids`、`/api/mail_details` 与 `/api/mail_flags` 接口，统一支持 `INBOX` / `SENT`、稳定游标和持久化系统标记；IMAP 改用轻量游标与批量详情接口，避免按 offset 扫描整个邮箱并补齐发件箱访问
...
- fix: |依赖安全| 将前端 Vite 从 7.3.5 升级到 7.3.6，并将传递依赖 esbuild 锁定为 0.28.1，修复 GHSA-g7r4-m6w7-qqqr（Windows 开发服务器可被利用读取任意本地文件，low）；升级后 `pnpm audit` 为 0
- fix: |认证安全| 用户与地址密码改为随机盐 PBKDF2-SHA256 服务端哈希，旧密码在成功登录时渐进升级；用户和地址密码登录增加基于 D1 的账号/IP 退避限流，并统一失败响应以减少账号枚举风险
- fix: |用户注册| 验证码改用 Web Crypto CSPRNG，并通过 D1 challenge 原子预占和一次性消费；邮箱按大小写无关方式规范化，已注册用户在发信前即被拒绝，避免重复注册继续收到验证码，并使用批处理保证用户、默认角色与 challenge 的一致性
- fix: |发件| `/api/send_mail`、`/external/api/send_mail` 与管理员发件路径增加 payload 绑定的幂等账本；余额及日/月额度改为条件式原子预占，供应商发送失败时执行补偿，避免并发重试造成重复发信、重复扣费或负余额
- fix: |收件| 为入站邮件增加确定性 `delivery_key` 和 D1 唯一约束以原子去重；存储失败会返回临时投递失败，让上游重试，而不再把可恢复故障永久拒收
- fix: |错误处理| Worker 顶层异常统一返回带 request ID 的脱敏 500 响应，公开及外部发件接口不再回显供应商或内部错误详情
- fix: |随机性安全| 地址密码、随机邮箱名、随机子域名和随机域名选择改用无模偏差的 Web Crypto CSPRNG；OAuth state 改用 256 位随机值

### Testing

- test: |Worker| 新增认证令牌轮换、PBKDF2、注册 challenge、重复注册拦截、登录限流、发件幂等与额度预占、入站重试去重、`INBOX` / `SENT` 游标、邮件标记和错误边界回归测试
...
- ci: |仓库治理| 移除可由公开评论触发、拥有写权限且执行浮动 `@main` 代码的第三方 PR Agent；增加独立构建验证、Dependabot 与 CodeQL 扫描
...
- fix: |GitHub Actions| 后端部署完成后自动将 GitHub Secrets 中的 Resend/SMTP 配置写回 Worker secrets，支持 `RESEND_TOKEN_<DOMAIN>` 域名级发信 token，避免自动部署后 Resend/SMTP 发信能力丢失
- fix: |GitHub Actions| `Upstream Sync` 改为 merge 上游 `main` 并保留本仓库自动部署适配文件，避免 fork 本地配置被强制覆盖后无法继续自动更新
